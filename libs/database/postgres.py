...
from libs.core.config import DATABASE_URL
...
class PostgresStorage:
    ...
    def get_existing_fingerprints(self, fingerprints: list[str]) -> set[str]:
        """
        Given a list of fingerprints, return the set of those that already exist in the DB.
        ...
        with self.get_session() as session:
            # Check in batches of 500 to avoid SQL limits
            existing = set()
            for i in range(0, len(fingerprints), 500):
                batch = fingerprints[i:i+500]
                results = session.query(JobRecord.fingerprint).filter(
                    JobRecord.fingerprint.in_(batch)
                ).all()
