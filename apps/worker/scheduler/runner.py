...
from libs.core.config import SCRAPE_INTERVAL_HOURS
...
async def run_pipeline(
    ...
    # ── Step 1.5: Incremental Filter (Deduplication) ────────────────────────
    # Only process jobs that don't already exist in our database
    ...
    if use_db:
        try:
            ...
            fingerprints = [j.fingerprint for j in raw_jobs]
            existing_fps = pg.get_existing_fingerprints(fingerprints)
            
            new_raw_jobs = [j for j in raw_jobs if j.fingerprint not in existing_fps]
            skipped_count = len(raw_jobs) - len(new_raw_jobs)
            
            if skipped_count > 0:
                logger.info(f"[Pipeline] ⏩ Skipping {skipped_count} jobs already in database.")
        except Exception as e:
            logger.warning(f"[Pipeline] Incremental filter failed (will process all): {e}")
            new_raw_jobs = raw_jobs
    else:
        new_raw_jobs = raw_jobs

    if not new_raw_jobs:
        logger.info("[Pipeline] ✓ All scraped jobs already exist in database. Done.")
        return {"run_id": run_id, "scraped_raw": total_scraped, "processed": 0, "saved": 0}

    # ── Step 2: NLP processing ────────────────────────────────────────────────
    nlp = NLPPipeline(use_multiprocessing=len(new_raw_jobs) > 50)
    processed_jobs = nlp.run(new_raw_jobs)
